...
from manuscript_narrative_clusters import result_display_claim_ids, result_display_item_ids
...
def test_results_section_brief_covers_display_backed_results() -> None:
    ...
    assert results["claim_packet_count"] == len(result_display_claim_ids())
    assert results["narrative_cluster_count"] == len(result_display_item_ids())
